...
local function handle_post_res(res)
	if res == 0 then
		vim.notify(
			"aw-watcher-nvim: Failed to connect to aw-server, logging will be disabled. You can retry to connect with :AWStart",
	...
	elseif res >= 100 and res < 300 or res == 304 then
		...
		vim.notify(
			string.format(
				"aw-watcher-nvim: aw-server did not accept our request with status code %d. See aw-server logs for reason or stop aw-watcher-vim with :AWStop",
				res
			),
			vim.log.levels.ERROR
		)
